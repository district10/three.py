...
class TestLineMaterials(Base):
    ...
    def initialize(self):
        ...
        # line-based material
        linePoints = [[-6, 0, -4], [6, 0, -4], [6, 0, 4], [-6, 0, 4],
                      [-6, 0, -4]]
        ...
        dashedMat = LineDashedMaterial(
            lineWidth=1, dashLength=0.2, gapLength=0.1)
        ...
        # solid red
        hilbert1 = Mesh(
            LineGeometry(hilbertPoints), LineBasicMaterial(color=[1, 0, 0]))
        ...
        for i in range(len(hilbertPoints)):
            vertexColorDataA.append(
                colorsys.hsv_to_rgb(i / len(hilbertPoints), 1, 1))
        ...
        # dashed red
        hilbert4 = Mesh(
            LineGeometry(hilbertPoints),
            LineDashedMaterial(color=[1, 0, 0], dashLength=0.2, gapLength=0.1))
        ...
        vertexColorDataB = []
        for i in range(len(hilbertPoints)):
            vertexColorDataB.append(
                colorsys.hsv_to_rgb(1, 0, i / len(hilbertPoints)))
