...
class TestViewports(Base):
    ...
    def initialize(self):
        ...
        starTexture = OpenGLUtils.initializeTexture(f"{pwd}/images/stars.jpg")
        stars = Mesh(
            SphereGeometry(200, 64, 64),
            SurfaceBasicMaterial(texture=starTexture))
        self.scene.add(stars)

        sunTexture = OpenGLUtils.initializeTexture(f"{pwd}/images/sun.jpg")
        earthTexture = OpenGLUtils.initializeTexture(f"{pwd}/images/earth.jpg")

        self.sun = Mesh(SphereGeometry(),
                        SurfaceBasicMaterial(texture=sunTexture))
        self.scene.add(self.sun)

        self.earth = Mesh(
            SphereGeometry(radius=0.5),
            SurfaceBasicMaterial(texture=earthTexture))
        ...
        # HUD setup
        self.hudScene = Scene()
        self.hudCamera = OrthographicCamera(
            left=0, right=self.w, bottom=0, top=self.h)
        ...
        self.quad1 = Sprite(
            SpriteMaterial(
                size=[hudImage.width, hudImage.height],
                anchor=[0, 0],
                texture=hudTexture1))
    ...
    def update(self):
        ...
        if self.input.resize():
            ...
            self.hudCamera.setViewRegion(
                left=0, right=size["width"], bottom=0, top=size["height"])
        ...
        self.earth.transform.setPosition(2 * cos(self.time), 0,
                                         2 * sin(self.time))
