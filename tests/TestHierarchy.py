...
class TestHierarchy(Base):
    ...
    def initialize(self):
        ...
        moonTexture = OpenGLUtils.initializeTexture(f"{pwd}/images/moon.jpg")
        ...
        self.sun = Mesh(
            SphereGeometry(1), SurfaceBasicMaterial(texture=sunTexture))
        ...
        self.earth = Mesh(
            SphereGeometry(0.5), SurfaceBasicMaterial(texture=earthTexture))
        self.earthGroup.add(self.earth)

        self.moon = Mesh(
            SphereGeometry(0.25), SurfaceBasicMaterial(texture=moonTexture))
    ...
    def update(self):
        ...
        earthX = earthOrbitDistance * cos(
            self.time * 2 * pi / earthOrbitDuration)
        earthZ = earthOrbitDistance * sin(
            self.time * 2 * pi / earthOrbitDuration)
