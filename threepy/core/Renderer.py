...
class Renderer(object):
    ...
    def render(self,
        ...
        if (renderTarget == None):
            ...
            glViewport(self.left, self.bottom, self.screenWidth,
                       self.screenHeight)
        else:
            # set render target properties
            glBindFramebuffer(GL_FRAMEBUFFER, renderTarget.framebufferID)
            glViewport(self.left, self.bottom, renderTarget.width,
                       renderTarget.height)
        ...
        for child in meshList:  # scene.children:
            ...
            if projectionMatrixVarID != -1:
                glUniformMatrix4fv(projectionMatrixVarID, 1, GL_TRUE,
                                   camera.getProjectionMatrix())

            viewMatrixVarID = glGetUniformLocation(ID, "viewMatrix")
            if viewMatrixVarID != -1:
                glUniformMatrix4fv(viewMatrixVarID, 1, GL_TRUE,
            ...
            for light in lightList:
                ...
                glUniform1i(
                    glGetUniformLocation(ID, lightName + ".isAmbient"),
                    light.isAmbient)
                glUniform1i(
                    glGetUniformLocation(ID, lightName + ".isDirectional"),
                    light.isDirectional)
                glUniform1i(
                    glGetUniformLocation(ID, lightName + ".isPoint"),
                    light.isPoint)
                glUniform3f(
                    glGetUniformLocation(ID, lightName + ".color"),
                    light.color[0], light.color[1], light.color[2])
                glUniform1f(
                    glGetUniformLocation(ID, lightName + ".strength"),
                    light.strength)
                position = light.transform.getPosition()
                glUniform3f(
                    glGetUniformLocation(ID, lightName + ".position"),
                    position[0], position[1], position[2])
                direction = light.direction  # apply rotation transform!
                glUniform3f(
                    glGetUniformLocation(ID, lightName + ".direction"),
