...
class ParticleGeometry(Geometry):
    ...
    def __init__(self, particlePositionData, particleColorData,
                 particleOpacityData, particleSizeData, particleAliveData):
        super().__init__()
...
class ParticleEngine(Mesh):
    ...
    def initializeParticle(self, particle):
        ...
        particle.color = RandomUtils.randomBoxVec3(self.colorBase,
                                                   self.colorSpread)
        particle.opacity = RandomUtils.randomFloat(self.opacityBase,
                                                   self.opacitySpread)
    ...
    def update(self, dt):
        ...
        for particle in self.particleList:
            ...
            if particle.alive == 1:
                ...
                self.particleGeometry.attributeData["particlePosition"][
                    "value"][particle.index] = particle.position
                self.particleGeometry.attributeData["particleColor"]["value"][
                    particle.index] = particle.color
                self.particleGeometry.attributeData["particleOpacity"]["value"][
                    particle.index] = particle.opacity
                self.particleGeometry.attributeData["particleSize"]["value"][
                    particle.index] = particle.size
                self.particleGeometry.attributeData["particleAlive"]["value"][
                    particle.index] = particle.alive

        # flag geometry attributes so updated data is resent to buffers
        self.particleGeometry.attributeData["particlePosition"][
            "needsUpdate"] = True
        self.particleGeometry.attributeData["particleColor"][
            "needsUpdate"] = True
        self.particleGeometry.attributeData["particleOpacity"][
            "needsUpdate"] = True
        self.particleGeometry.attributeData["particleSize"][
            "needsUpdate"] = True
        self.particleGeometry.attributeData["particleAlive"][
            "needsUpdate"] = True
