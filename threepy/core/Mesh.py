...
from threepy.core import Object3D
    ...
    def render(self):
        ...
        for name, data in self.geometry.attributeData.items():
            ...
            if (attributeVarID != -1):
                ...
                if data["type"] == "float":
                    glVertexAttribPointer(attributeVarID, 1, GL_FLOAT, False, 0,
                                          None)
                elif data["type"] == "vec2":
                    glVertexAttribPointer(attributeVarID, 2, GL_FLOAT, False, 0,
                                          None)
                elif data["type"] == "vec3":
                    glVertexAttribPointer(attributeVarID, 3, GL_FLOAT, False, 0,
                                          None)
                elif data["type"] == "vec4":
                    glVertexAttribPointer(attributeVarID, 4, GL_FLOAT, False, 0,
                                          None)
                else:
                    raise Exception("Attribute " + data['name'] +
        ...
        for name, data in self.material.uniformData.items():
            ...
            if (uniformVarID != -1):
                if data["type"] == "bool":
                    ...
                    glUniform2f(uniformVarID, data["value"][0],
                                data["value"][1])
                elif data["type"] == "vec3":
                    glUniform3f(uniformVarID, data["value"][0],
                                data["value"][1], data["value"][2])
                elif data["type"] == "vec4":
                    glUniform4f(uniformVarID, data["value"][0],
                                data["value"][1], data["value"][2],
                                data["value"][3])
                    ...
                    raise Exception("Uniform " + data['name'] +
                                    " has unknown type " + data['type'])
