...
class Tween(object):
    ...
    def evaluate(self, time):
        ...
        percent = Tween.percentFloat(self.timeList[index - 1],
                                     self.timeList[index], time)

        # return interpolated value, according to value type
        if self.valueType == "float":
            return Tween.lerpFloat(self.valueList[index - 1],
                                   self.valueList[index], percent)
        elif self.valueType == "vec3":
            return Tween.lerpVec3(self.valueList[index - 1],
                                  self.valueList[index], percent)
        else:
            raise Exception("Tween.evaluate() - unknown value type: " +
                            self.valueType)
